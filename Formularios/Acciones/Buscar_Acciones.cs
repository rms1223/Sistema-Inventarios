...
namespace InventarioFod.Formularios.Acciones
{
    public partial class Buscar_Acciones : Form
    {
        ...
        public Buscar_Acciones(Label orden_tra, TextBox institu)
        {
            InitializeComponent();
            base_datos = Conexion_db_Mysql.getInstance;
            dataGridView1.DataSource = base_datos.Obtener_Estados_Acciones(0);
            orden = orden_tra;
            institucion = institu;
        }
        public Buscar_Acciones(TextBox box_orden)
        {
            InitializeComponent();
            base_datos = Conexion_db_Mysql.getInstance;
            dataGridView1.DataSource = base_datos.Obtener_Estados_Acciones(0);
        ...
        public Buscar_Acciones(Label box_orden)
        {
            InitializeComponent();
            base_datos = Conexion_db_Mysql.getInstance;
            dataGridView1.DataSource = base_datos.Obtener_Estados_Acciones(0);
        ...
        {
            dataGridView1.DataSource = base_datos.Obtener_Estados_Acciones(Convert.ToInt32(txt_orden.Text));
        }
